...
get_compose_file_args() {
  ...
  if [ "$use_cn" = "true" ] && [ -f "$MAIN_COMPOSE_CN" ]; then
    compose_args="$compose_args -f $MAIN_COMPOSE_CN"
  fi
  
  if [ "$mode" = "source" ]; then
    echo "[WARN] --source flag is deprecated; docker-compose.yml already builds from local source." >&2
  fi
...
Commands:
  ...
  build <service>                     Rebuild service (api|platform|web|widget|all)
...
Notes:
  ...
  - Pass --source to force local rebuild (deprecated; docker-compose.yml already builds from source).
...
show_install_complete_message() {
  ...
  if [ -n "$userland_proxy" ]; then
    if [ "$userland_proxy" = "true" ]; then
      ...
      echo "See the README reverse-proxy section for enabling real IP headers."
      echo ""
    fi
  fi
}
...
cmd_build() {
  ...
  case "$target" in
    ...
    *)
      ...
      echo "Supported: api, platform, web, widget, all" >&2
      exit 1
      ;;
  esac

  local compose_file_args
  compose_file_args=$(get_compose_file_args "source" "$USE_CN_MIRROR") || compose_file_args="-f $MAIN_COMPOSE_IMAGE"
