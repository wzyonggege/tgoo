...
def _get_user_info_blocking(bot_token: str, user_id: str) -> dict:
    client = WebClient(token=bot_token)
...
def _get_dm_channel_blocking(bot_token: str, user_id: str) -> str:
    client = WebClient(token=bot_token)
...
async def slack_get_user_info(
    ...
    """
    try:
        return await asyncio.to_thread(_get_user_info_blocking, bot_token, user_id)
    except SlackApiError as e:
...
async def slack_get_dm_channel(
    ...
    """
    try:
        return await asyncio.to_thread(_get_dm_channel_blocking, bot_token, user_id)
    except SlackApiError as e:
