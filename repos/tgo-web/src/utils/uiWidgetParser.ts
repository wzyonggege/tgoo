...
import type { WidgetData } from '@/components/chat/widgets';
...
export function replaceUIWidgetsWithPlaceholders(
  ...
  // Debug: Check if content contains tgo-ui-widget
  const hasTgoWidget = content.includes('tgo-ui-widget');
  const hasTripleBacktick = content.includes('```');
  console.log('[UIWidgetParser] Content check:', {
  ...
  // Test regex match
  const testMatch = UI_WIDGET_REGEX.exec(content);
  console.log('[UIWidgetParser] Regex test match:', testMatch ? 'Found' : 'Not found');
  UI_WIDGET_REGEX.lastIndex = 0;

  const processedContent = content.replace(
    UI_WIDGET_REGEX,
    (match, jsonContent) => {
      console.log('[UIWidgetParser] Matched widget block, JSON length:', jsonContent?.length);
      try {
